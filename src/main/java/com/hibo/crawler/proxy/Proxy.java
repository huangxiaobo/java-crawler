package com.hibo.crawler.proxy;
...
public class Proxy implements Delayed, Serializable {
  ...
  public Proxy(String ip, int port, long timeInterval) {
    ...
    this.timeInterval =
        TimeUnit.NANOSECONDS.convert(timeInterval, TimeUnit.MILLISECONDS) + System.nanoTime();
  }
  ...
  public boolean isDiscardProxy() {
    if (failureTimes > 3) {
      if (1.0 * failureTimes / (failureTimes + successfulTimes) > 0.6) {
        return true;
      }
    }
    return false;
